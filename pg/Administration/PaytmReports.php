...
$res = sql_query("SELECT * FROM `tb_virtualtxn` WHERE client_orderid='0' AND id='".$tid."'  ");
$row = sql_num_rows($res);	
$result = sql_fetch_array($res);
if($row==1){

if(sql_query("UPDATE `tb_virtualtxn` SET client_orderid='".time()."' WHERE id='".$result['id']."' ")){
...
$query = sql_query("SELECT * FROM `tb_virtualtxn` WHERE client_orderid='0' || client_orderid='' ");
...
<button class="btn bg-mix text-white btn-sm" name="completed_all" type="submit">Completed All <i class="fa fa-check-circle"></i></button>
                      ...
                      <tbody>
<?php
$sql = "SELECT * FROM `tb_virtualtxn` ORDER BY `id` DESC ";	
...
$concat = '';
if($search_input!=''){
$concat	= "WHERE CONCAT(`txn_id`,`bank_ref_num`) LIKE '%$search_input%' AND ";
}


$sql = "SELECT * FROM `tb_virtualtxn` ORDER BY `id` DESC LIMIT $rows_input ";	
...
$usql = sql_query("SELECT * FROM `tb_partner` WHERE id='".$rows['user_id']."'  ");
$user_data = sql_fetch_array($usql);
...
?>						  
						<tr>
                          <td>#<?php echo $rows['txn_id'];?><br><span style="font-size:12px;"><?php echo $user_data['login_id'];?><br>(<?php echo $user_data['name'];?>)</span></td>
                          <td><?php echo $rows['credited_at'];?></td>
                          <td><?php echo strtoupper($rows['bene_account_no']);?><br>Received - <?php echo $rows['payment_type'];?></td>
                          <td><?php echo strtoupper($rows['rmtr_account_no']);?><br><?php echo strtoupper($rows['rmtr_full_name']);?></td>
                          ...
                          <td><?=$status?><br><?=$client_orderid?></td>
                        </tr>
<?php
}
}
